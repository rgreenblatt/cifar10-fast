...
def table_formatter(val, is_title=False, col_width=12, formats=None):
    ...
    type_ = lambda val: float if isinstance(val,
                                            (float, np.float)) else type(val)
    return (formats['title'] if is_title else formats.get(
        type_(val), formats['default'])).format(val, w=col_width)
...
class Table():
    ...
    def df(self):
        return pd.DataFrame([{'_'.join(p): v
                              for p, v in path_iter(row)} for row in self.log])
...
def build_graph(net):
    ...
    resolve_input = lambda rel_path, path, idx: normpath(
        sep.join((path, '..', rel_path))) if isinstance(
            rel_path, str) else flattened[idx + rel_path][0]
    return {
        path:
...
class ColorMap(dict):
    ...
    def _repr_html_(self):
        ...
        return '<style>' + css + '</style>' + ''.join(
            (s.format(color, text) for text, color in self.items()))
...
class DotGraph():
    def __init__(self, graph, size=15, direction='LR'):
        ...
        self.edges = [(src, dst, {}) for dst, (_, inputs) in graph.items()
                      for src in inputs]
        self.size, self.direction = size, direction

    def dot_graph(self, **kwargs):
        return make_dot_graph(self.nodes,
                              self.edges,
                              size=self.size,
